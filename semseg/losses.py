...
def compute_eraft_flow_loss(flow_t0_t1, gt_flow, gamma=0.8):
    # flow_t0_t1: (iter, B, 2, H, W)
    # gt_flow: (B, 2, H, W)
    iter_steps = len(flow_t0_t1)
    loss = 0.0
    
    for k in range(iter_steps):
        # Compute L1 distance
        l1_distance = torch.abs(gt_flow - flow_t0_t1[k]).sum(dim=(1, 2, 3))  # Sum over channels, height, width
        # Apply weighting
        weight = gamma ** (iter_steps - 1 - k)
        loss += weight * l1_distance

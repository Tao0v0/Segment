...
class DSEC_Flow(Dataset):
    ...
    def __init__(self, root: str = 'data/DSEC', split: str = 'train', n_classes: int = 11, transform = None, modals = ['img', 'event'], case = None, duration: int=None, flow_net_flag: bool=False, dataset_type: str=None) -> None:
        ...
        self.flow_gt_dirname = f'/flow_t0_t{self.time_window}'
        # dt = 1
        ...
        # self.n_classes = 13
        # 获取所有场景
        scenes = os.listdir(os.path.join(root, self.flow_gt_dirname[1:], split))
        self.files = []
        
        # 遍历每个场景
        for scene in scenes:
            flow_scene_dir = os.path.join(root, self.flow_gt_dirname[1:], split, scene)
            seg_scene_dir = os.path.join(root, self.seg_gt_dirname[1:], split, scene)
            
            # 获取所有 Flow 文件
            flow_files = [f for f in os.listdir(flow_scene_dir) if f.endswith('.png')]
            
            # 筛选符合条件的 Flow 文件
            for flow_file in flow_files:
                # 提取 ID（假设文件名为 000784.png）
                flow_id = int(flow_file.split('.')[0])  # 提取 "000784" 并转换为整数
                
                # 构造对应的 Seg 文件名（ID + 1）
                seg_id = flow_id + 5  # 5 for SDSEC and 1 for DSEC
                seg_file = f"{seg_id:06d}_gtFine_labelTrainIds11.png"  # 格式化为 6 位数字
                
                # 检查 Seg 文件是否存在
                seg_path = os.path.join(seg_scene_dir, seg_file)
                if os.path.exists(seg_path):
                    self.files.append(seg_path)
        ...
        flow_files = sorted(glob.glob(os.path.join(*[root, self.flow_gt_dirname[1:], split, '*', f'*.png'])))
        print(f"Found {len(flow_files)-len(scenes)} {split} flow images.")
        # last file name
        print(f"Last file name: {flow_files[-1]}")
        print(f"Found {len(self.files)} {split} images.")
        # print(f"Last file name: {self.files[-1]}")
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, Tensor]:
        ...
        if self.time_window != 0:
            ...
            sample.append(flow)
        return seq_name, seq_idx, sample
